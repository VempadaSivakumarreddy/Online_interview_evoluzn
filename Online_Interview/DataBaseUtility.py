...
def insertQuestions():
    ...
    lastPrimaryKey =  myresult[-1]['idquestions'] if len(myresult) > 0 else 0


    with open("D:\Quiz\env\env\Questions.csv" ,mode="r",encoding="utf-8") as questionContainer:
    ...
    for indx ,row in enumerate(questionsRowWise):
       # print(indx,row)
        ...
        QuestionType = asList[4].strip()

        print(asList,QuestionType)
        query = "INSERT INTO `quiz`.`questions` (`idquestions`, `questionid`, `question`, `options`, `CorrectAnswer`, `Subject`,`QuestionType`) VALUES ('{id}', '{idq}', '{qsn}', '{opn}', '{cAns}', '{sub}','{qType}')".format(id=primaryKey,idq=str(primaryKey),qsn =question,opn = options,cAns = correctAns,sub = Subject,qType = QuestionType)

        #print(query)QuestionType
        cursor.execute(query)
        conn.commit()
        # mycursor.execute("SELECT * FROM customers")
        # cursor.excecute("INSERT INTO `quiz`.`questions` (`idquestions`, `questionid`, `question`, `options`, `CorrectAnswer`, `Subject`,'QuestionType) VALUES ('3', '3', '2+3', '4,3,9', '5', 'Math')")
